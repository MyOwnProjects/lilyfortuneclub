...
class Task_model extends Base_model{
	...
	public function get_list($where = '', $sort = array(), $limit = ''){
		...
		$sql = "SELECT tasks.*, IF(tasks.tasks_due_date IS NULL, '9999-01-01', tasks.tasks_due_date) AS tasks_due_date, users.first_name, users.last_name, users.nick_name "
	...
	public function insert($case_no, $name, $subject, $detail, $type, $source, $priority, $create, $due_date, $status){
		$sql = "INSERT INTO tasks (tasks_case_no, tasks_name, tasks_subject, 
			...
			'".(isset($detail) ? addslashes($detail) : '')."', '$status', '$type', '$source', '$priority', 
