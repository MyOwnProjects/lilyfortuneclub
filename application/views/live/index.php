...
.resource-list ul:not(.breadcrumb) li{list-style:square;padding:0}
	...
	success:function(data){
		...
		for(var i = 0; i < data.length; ++i){
			...
			if(img_wrappers.length > 0){
				$_a.clone().append('<img src="' + $(img_wrappers[0]).children('img').attr('src').substr(6) + '">').appendTo($_thumbnail);
			}
