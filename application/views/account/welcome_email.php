...
<div style="margin:0 auto;max-width:1000px;padding:20px 0 80px 0;">
	<h2 class="text-center">Welcome Email</h2>
		<div class="form-group">
			<label>Select the member <span style="font-weight:normal !important">(Enter the name or code to select)</span></label>
			<select class="form-control control-sm" data-live-search='true' id="member-select">
				<?php 
				foreach($members as $m){
				?>
				<option value="<?php echo $m['membership_code'];?>"><?php echo $m['name'].' - '.$m['membership_code'];?></option>
				<?php 
				}
				?>
			</select>
			
			<button style="margin:10px 0 20px 0" class="btn btn-success" onclick="get_member_info();">Select</button>
		</div>
		<textarea id="email-template" style="width:100%;height:100px"></textarea>	
</div>
<script>
